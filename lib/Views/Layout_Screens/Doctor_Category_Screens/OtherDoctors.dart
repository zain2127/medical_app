...
class OtherDoctorScreen extends StatefulWidget {
  const OtherDoctorScreen({Key? key}) : super(key: key);

  @override
  State<OtherDoctorScreen> createState() => _OtherDoctorScreenState();
}

class _OtherDoctorScreenState extends State<OtherDoctorScreen> {
  ...
  Future getDataFromApi() async {
    ...
    if (response.statusCode == 200) {
      ...
      final OtherDoctor=data["doctor"].where(
              (doctor) => doctor["specialtiy"] != "surgeon" && doctor["specialtiy"] != "Surgeon" && doctor["specialtiy"] != "dentist"  && doctor["specialtiy"] != "Dentist"
      ).toList();
      return OtherDoctor;
    } else {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: AppBar(
          ...
          title: const Text("Other Doctor List",style: TextStyle(color: Colors.black,fontWeight: FontWeight.bold),),
          centerTitle: true,
          elevation: 0,
